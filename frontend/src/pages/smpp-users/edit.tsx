...
import type { SmppUser, SmppUserUpdateRequest } from '@/types';
...
export default function SmppUsersEdit() {
  ...
  const [formData, setFormData] = useState<SmppUserUpdateRequest>({
    system_id: '',
    password: '',
    ...
    const fetchSmppUser = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          max_connection_speed: data.data.max_connection_speed,
          is_active: data.data.is_active,
          
          // MT Messaging Credentials
          mt_src_addr: data.data.mt_src_addr || '',
          mt_http_throughput: data.data.mt_http_throughput || 'ND',
          mt_balance: data.data.mt_balance || 'ND',
          mt_smpps_throughput: data.data.mt_smpps_throughput || 'ND',
          mt_sms_count: data.data.mt_sms_count || 'ND',
          mt_early_percent: data.data.mt_early_percent || 'ND',
          mt_priority_filter: data.data.mt_priority_filter || '^[0-3]$',
          mt_content_filter: data.data.mt_content_filter || '.*',
          mt_src_addr_filter: data.data.mt_src_addr_filter || '.*',
          mt_dst_addr_filter: data.data.mt_dst_addr_filter || '.*',
          mt_validity_period_filter: data.data.mt_validity_period_filter || '^\\d+$',
          mt_http_send: data.data.mt_http_send,
          mt_http_dlr_method: data.data.mt_http_dlr_method,
          mt_http_balance: data.data.mt_http_balance,
          mt_smpps_send: data.data.mt_smpps_send,
          mt_priority: data.data.mt_priority,
          mt_http_long_content: data.data.mt_http_long_content,
          mt_src_addr_auth: data.data.mt_src_addr_auth,
          mt_dlr_level: data.data.mt_dlr_level,
          mt_http_rate: data.data.mt_http_rate,
          mt_validity_period: data.data.mt_validity_period,
          mt_http_bulk: data.data.mt_http_bulk,
          mt_hex_content: data.data.mt_hex_content,
        });
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.put(`/smpp-users/${id}`, formData);
      navigate('/smpp-users');
    } catch (error) {
